...
ローカルファーストな Next.js + Electron デスクトップアプリの中に、ビジュアルなストーリーブループリント、ノード単位の FlowTimeline エディター、ローカル素材管理、インタラクティブプレビュー、エクスポートツール、AI 支援の制作レイヤーを組み合わせています。プロジェクト、インポートしたメディア、タイムラインデータ、生成パッケージはすべて手元のマシンに保存されます。アカウントシステム、ホスト型データベース、クラウドストレージには依存しません。

![OpenFMV エディター概要](./public/readme/openfmv-editor-overview.jpg)
...
3. `/editor` のブループリントグラフでストーリー構造を作ります。
4. `/nodes` で FlowTimeline を使い、各シーンのメディアトラックとインタラクショントラックを編集します。
5. インタラクティブ再生と分岐動作をプレビューします。
6. ストーリーを共有またはテストできる状態になったら、ローカル再生可能なパッケージを書き出します。
...
![OpenFMV ストーリーブループリント](./public/readme/openfmv-editor-overview.jpg)

### インタラクティブ再生プレビュー

視聴者がストーリー内をどのように進むかをプレビューできます。ボタン選択、シーン遷移、インタラクティブ再生を文脈の中で確認する場所です。

![OpenFMV 再生プレビュー](./public/readme/openfmv-play-preview.jpg)
...
- **ブループリントグラフ編集:** ノード、ハンドル、エッジ、分岐出力で非線形ストーリーフローを構築できます。
