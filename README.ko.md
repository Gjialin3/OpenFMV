...
로컬 우선 Next.js + Electron 데스크톱 앱 안에 시각적 스토리 블루프린트, 노드 단위 FlowTimeline 에디터, 로컬 에셋 관리, 인터랙티브 미리보기, 내보내기 도구, AI 보조 제작 레이어를 결합합니다. 프로젝트, 가져온 미디어, 타임라인 데이터, 생성된 패키지는 모두 사용자의 기기에 저장됩니다. 계정 시스템, 호스팅 데이터베이스, 클라우드 스토리지에 의존하지 않습니다.

![OpenFMV 에디터 개요](./public/readme/openfmv-editor-overview.jpg)
...
3. `/editor` 블루프린트 그래프에서 스토리 구조를 만듭니다.
4. `/nodes`에서 FlowTimeline으로 각 장면의 미디어 트랙과 인터랙션 트랙을 편집합니다.
5. 인터랙티브 재생과 분기 동작을 미리 봅니다.
6. 스토리를 공유하거나 테스트할 준비가 되면 로컬 재생 가능 패키지를 내보냅니다.
...
![OpenFMV 스토리 블루프린트](./public/readme/openfmv-editor-overview.jpg)

### 인터랙티브 재생 미리보기

시청자가 스토리 안에서 어떻게 이동하는지 미리 봅니다. 버튼 선택, 장면 전환, 인터랙티브 재생을 맥락 안에서 확인하는 공간입니다.

![OpenFMV 재생 미리보기](./public/readme/openfmv-play-preview.jpg)
...
- **블루프린트 그래프 편집:** 노드, 핸들, 엣지, 분기 출력으로 비선형 스토리 흐름을 구성합니다.
